...
namespace EpsilonServer.EpsilonClientAPI.Editor
{
    public class EpsilonEditor : EditorWindow
    {
        ...
        {
            Selection.activeObject = AssetDatabase.LoadMainAssetAtPath("Assets/POPBlocks/Server/Resources/Scriptable/ServerSettings.asset");
        }  
            ...
            if (GUILayout.Button("Register game") && email != "")
            {
                EpsilonClientAPI.SendEditorRequiest("/system/register_game", "game_name="+game_name+"&email="+email+"&match3=Yes" +"&send_mail=Yes" + "&invoice=" + invoice, result =>
                {
                    if (result != null)
                    {
                        ...
                        var serverSettings = AssetDatabase.LoadAssetAtPath<ServerSettings>("Assets/POPBlocks/Server/Resources/Scriptable/ServerSettings.asset");
